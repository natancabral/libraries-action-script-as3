package GNNC.UI.gnncNotification
{
	 ...
	 * 
	 * @eventType com.charlesbihis.engine.notification.gnncNotificationEvent.NOTIFICATION_CLOSE
	 * 
	 * @see com.charlesbihis.engine.notification.gnncNotificationEvent
	 */
	[Event(name="notificationCloseEvent", type="GNNC.UI.gnncNotification.event.gnncNotificationEvent")]
	
	/**
	 * Event broadcast when an action to close all active notifications has been invoked.
	 * 
	 * @eventType com.charlesbihis.engine.notification.gnncNotificationEvent.CLOSE_ALL_NOTIFICATIONS
	 * 
	 * @see com.charlesbihis.engine.notification.gnncNotificationEvent
	 */
	[Event(name="closeAllNotificationsEvent", type="GNNC.UI.gnncNotification.event.gnncNotificationEvent")]
	
	/**
	 * Event broadcast when the user has gone idle.
	 * 
	 * @eventType com.charlesbihis.engine.notification.gnncNotificationEvent.USER_IS_IDLE
	 * 
	 * @see com.charlesbihis.engine.notification.gnncNotificationEvent
	 */
	[Event(name="userIsIdleEvent", type="GNNC.UI.gnncNotification.event.gnncNotificationEvent")]
	
	/**
	 * Event broadcast when the user has returned from idle.
	 * 
	 * @eventType com.charlesbihis.engine.notification.gnncNotificationEvent.USER_IS_PRESENT
	 * 
	 * @see com.charlesbihis.engine.notification.gnncNotificationEvent
	 */
	[Event(name="userIsPresentEvent", type="GNNC.UI.gnncNotification.event.gnncNotificationEvent")]
	 ...
	 */
	public class gnncNotificationManager extends EventDispatcher
	{

		[Embed(source="GNNC/UI/gnncNotification/sound/drop.mp3")] 
		public var soundDrop:Class; 
		...
		private static const MINIMUM_TIME_BETWEEN_NOTIFICATION_SOUNDS:int = 10000;	// 10.000 = 10 seconds
		
		private var log:ILogger = Log.getLogger("GNNC.UI.gnncNotification.NotificationManager");
		 ...
		 * @param displayLength (Optional) Length, in seconds, that notifications will stay on the screen.  Defaults to gnncNotificationConst.DISPLAY_LENGTH_MEDIUM.
		 * @param displayLocation (Optional) Location on-screen (i.e. top-left, top-right, bottom-left, or bottom-right) where notifications are to appear.  Defaults to gnncNotificationConst.DISPLAY_LOCATION_AUTO.
		 * 
		 * @see com.charlesbihis.engine.notification.gnncNotificationConst
		 */
		public function gnncNotificationManager(defaultStyle:String='', defaultNotificationImage:String=null, defaultCompactNotificationImage:String=null, notificationSound:* = null, displayLength:int = gnncNotificationConst.DISPLAY_LENGTH_MEDIUM, displayLocation:String = gnncNotificationConst.DISPLAY_LOCATION_AUTO)
		{
			// initialize
			...
			addEventListener(gnncNotificationEvent.NOTIFICATION_CLOSE, notificationCloseHandler);
			...
			if (_displayLocation == gnncNotificationConst.DISPLAY_LOCATION_AUTO)
			{
				...
				if (os.indexOf("Mac") >= 0)
				{
					...
					_displayLocation = gnncNotificationConst.DISPLAY_LOCATION_TOP_RIGHT;
				}  // if statement
				else
				{
					log.info("Display location has detected an operating system of \"{0}\".  Setting default display location to bottom right.", os);
					_displayLocation = gnncNotificationConst.DISPLAY_LOCATION_BOTTOM_RIGHT;
				}  // else statement
			}  // if statement
			
			// load default style
			//var loadStyleEvent:IEventDispatcher = FlexGlobals.topLevelApplication.styleManager.loadStyleDeclarations2(defaultStyle);
			//loadStyleEvent.addEventListener(StyleEvent.COMPLETE, loadComplete());
			loadComplete();
			
			// load notification sound
			if (notificationSound is String && notificationSound != null)
			{
			...
			{
				//gnncAudio.play(notificationSound);

				var p:Sound = new soundDrop() as Sound;
				p.play();
			}
		 ...
		 */
		public function showNotification(notification:gnncNotificationValues):void
		{
		...
		{
			var notification:gnncNotificationValues = new gnncNotificationValues();
		...
		public function replayLatestFiveUpdates():void
		{
			...
			if (previousQueue.length == 0)
			{
				...
				var noUpdatesNotification:gnncNotificationValues = new gnncNotificationValues();
			...
			for (var i:int = 0; i < previousQueue.length; i++)
			{
				...
				var previousNotification:gnncNotificationValues = previousQueue.getItemAt(i) as gnncNotificationValues;
				var notification:gnncNotificationValues = new gnncNotificationValues();
		...
		public function loadStyle(style:String):void
		{
			...
			//loadStyleEvent.addEventListener(StyleEvent.COMPLETE, );
			loadComplete();
		}  // loadStyle
		 ...
		 * <code>gnncNotificationEvent.CLOSE_ALL_NOTIFICATIONS</code> event to do so.
		 * 
		 * @see GNNC.UI.gnncNotification.event.gnncNotificationEvent#CLOSE_ALL_NOTIFICATIONS
		 */
		public function closeAllNotifications():void
		{
			log.info("Dispatching gnncNotificationEvent.CLOSE_ALL_NOTIFICATIONS event");
			var NotificationEvent:gnncNotificationEvent = new gnncNotificationEvent(gnncNotificationEvent.CLOSE_ALL_NOTIFICATIONS);
		...
		private function showAll():void
		{
			...
			{
				// show it
				var notification:gnncNotificationValues = queue.getItemAt(0) as gnncNotificationValues;
		...
		private function userPresentHandler(event:Event):void
		{
			...
			_isUserIdle = false;
			dispatchEvent(new gnncNotificationEvent(gnncNotificationEvent.USER_IS_PRESENT));
			
			if (suppressedNotificationCount > 0)
			{
				// build summary notification
				var summaryNotification:gnncNotificationValues = new gnncNotificationValues();
		...
		private function userIdleHandler(event:Event):void
		{
			...
			_isUserIdle = true;
			dispatchEvent(new gnncNotificationEvent(gnncNotificationEvent.USER_IS_IDLE));
		}  // onIdle

...
package GNNC.UI.gnncTextInputMasked
{
	...
	import GNNC.UI.gnncTextInputMasked.MTISkin;
	 ...
	 * 
	 */
	public class gnncTextInputMasked extends TextInput
	{	
		//--------------------------------------------------------------------------
		//
		//  Constructor
		 ...
		 *  Constructor.
		 */
		public function gnncTextInputMasked()
		{
			super();
		...
		override protected function createChildren():void
		{
			...
			if(dateField)
			{
				...
				//addChildAt(embedTextField as DisplayObject,getChildIndex(textField));
			}
		}
		...
		private function interceptChar( event:TextEvent ) : void
		{
			...
			if( _position >= actualLength ) {
				...
				if(_autoAdvance && !isWorkingIncomplete())
				{
					...
					if((obj is gnncTextInputMasked) || (obj is TextInput) || (obj is TextArea))
						obj.setFocus();
				}
		...
		override protected function updateDisplayList(unscaledWidth:Number, unscaledHeight:Number):void
		{
			...
			if( maskUpdated ) {
				...
				//width = measureText("W").width *  actualLength + 2*getStyle("borderThickness") + 5; //gnnc nc
				width = measureText("W").width *  actualLength + 1*getStyle("borderThickness") + 1;
				workingUpdated = true;
			}
			...
			if(fontUpdated)
			{
				...
				//width = measureText("W").width *  actualLength + 2*getStyle("borderThickness") + 5;//gnnc nc
				width = measureText("W").width *  actualLength + 1*getStyle("borderThickness") + 0;
			}	
			if(dateField)
			{
				...
				{
					embedTextField = new UITextField();
					//addChildAt(DisplayObject(embedTextField),getChildIndex(textField));
				}
